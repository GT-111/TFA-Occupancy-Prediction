...
class TrajNet(nn.Module):
    ...
    def forward(self,obs_traj,map_traj=None,training=True):
        ...
        c_attn_mask = torch.not_equal(torch.max(torch.concat([obs_mask], dim=1).to(torch.int32),dim=-1)[0],0) #[batch,64] (last step denote the current)
        c_attn_mask = c_attn_mask.to(torch.float32)

        if self.no_attn:
            if self.double_net:
                concat_actors = torch.concat([obs], dim=1)
                obs = self.obs_norm(concat_actors)
                return obs,c_attn_mask
            else:
                return self.obs_norm(obs),c_attn_mask
        ...
        if self.double_net:
            ...
            val_obs = value[:,:obs_actors,:]

            value_flow = self.cross_attention[1](query=query, key=concat_actors, mask=attn_mask, training=training)
            val_obs_f = value_flow[:,:obs_actors,:]

            obs = obs + val_obs

            ogm = torch.concat([obs], dim=1)

            obs_f = obs + val_obs_f
            flow = torch.concat([obs_f], dim=1)

            return self.obs_norm(ogm), c_attn_mask
...
class TrajNetCrossAttention(nn.Module):
    ...
    def forward(self,pic_encode,obs_traj,map_traj=None,training=True):

        obs,traj_mask = self.traj_net(obs_traj,map_traj,training)
