    ...
    with torch.no_grad():
        ...
        for batch_idx, data in loop:
            ...
            if occupancy_flow_map_metrics_dict["vehicles_observed_occupancy_auc"] > best_auc:
                ...
                per_step_metrics = occupancy_flow_map_metrics.compute_occupancy_metrics(pred_observed_occupancy_logits, gt_observed_occupancy_logits, gt_occupancy_flow_map_mask, mean=False)
                def get_k_step_mean(metrics, k):
                    return {key: np.mean([metrics[key][step].cpu() for step in range(k)]) for key in metrics.keys()}
                # 5 step
                per_step_metrics_5 = get_k_step_mean(per_step_metrics, 5)
                # 10 step
                per_step_metrics_10 = get_k_step_mean(per_step_metrics, 10)
                # 15 step
                per_step_metrics_15 = get_k_step_mean(per_step_metrics, 15)
