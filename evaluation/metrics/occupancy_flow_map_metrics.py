...
class OccupancyFlowMapMetrics(Metrics):
	...
	def compute_occupancy_metrics(self,
		...
		batch_size, occupancy_flow_map_height, occupancy_flow_map_width, num_waypoints, _ = pred_observed_occupancy_logits.size()
		...
		for k in range(num_waypoints):
			...
			metrics_dict['vehicles_observed_occupancy_auc'].append(
				_compute_occupancy_auc(true_observed_occupancy * mask, pred_observed_occupancy * mask))
	...
	for k in range(num_waypoints):
		...
		if True:  # has_true_occluded_occupancy[k]:
			metrics_dict['vehicles_occluded_occupancy_auc'].append(
				_compute_occupancy_auc(true_occluded_occupancy * mask, pred_occluded_occupancy * mask)
			)
			metrics_dict['vehicles_occluded_occupancy_iou'].append(
				_compute_occupancy_soft_iou(true_occluded_occupancy * mask, pred_occluded_occupancy * mask)
			)
		...
		if not no_warp:
			...
			metrics_dict['vehicles_flow_warped_occupancy_iou'].append(
				_compute_occupancy_soft_iou(flow_grounded_pred_all_occupancy * mask, true_all_occupancy * mask)
			)
	# Compute means and return as proto message.
	metrics_dict = {k: _mean(v) for k, v in metrics_dict.items()}
	return metrics_dict
