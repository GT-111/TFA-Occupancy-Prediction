...
class FlowGuidedMultiHeadSelfAttention(nn.Module):
    def __init__(
        ...
        self.conv_offset_0 = nn.Conv2d(self.nc, self.nc, kernel_size=kk, stride=self.stride, padding="same", groups=self.n_groups)
        ...
        if self.fg:
            self.conv_offset_proj2 = nn.Conv2d(2, self.out_dim, kernel_size=1,stride=1)
        ...
        self.proj_out = nn.Conv2d(self.nc, self.out_dim,kernel_size=1, stride=1)
        ...
        dummy_x = torch.zeros((1,self.q_h,self.q_w,self.in_dim))
